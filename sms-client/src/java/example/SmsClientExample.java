...
public class SmsClientExample {
    public static void main(String[] args) throws Exception {
        ...
        m11.put("passwd", "gax6ChD0yft");
        m1.put("name", "testsec"); //secret name
        m1.put("values", m11); //values
        SmsResponse resp2 = sms.storeSecret("onap.new.test.sms0", m1);
        if ( resp2.getSuccess() ) {
        ...
        m22.put("username", "dbuser");
        m2.put("name", "user"); //secret name
        m2.put("values", m22); //values
        SmsResponse resp3 = sms.storeSecret("onap.new.test.sms0", m2);
        if ( resp3.getSuccess() ) {
        ...
        SmsResponse resp5= sms.retrieveSecret("onap.new.test.sms0", "testsec");
        if ( resp5.getSuccess() ) {
            System.out.println(resp5.getResponse());
            System.out.println(resp5.getResponseCode());
        }
        SmsResponse resp6= sms.retrieveSecret("onap.new.test.sms0", "user");
        if ( resp6.getSuccess() ) {
